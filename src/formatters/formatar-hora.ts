...
export const formatarHorasArredondadas = (value: string | number | null | undefined): string => {
...
export const formatarHorasTotaisSufixo = (value: string | number | null | undefined): string => {
    if (value == null) return '-';

    // Guarda o sinal
    const isNegative = typeof value === 'number' ? value < 0 : parseFloat(value.toString()) < 0;
    const absoluteValue =
        typeof value === 'number' ? Math.abs(value) : Math.abs(parseFloat(value.toString()));

    const { hours, minutes } = parseTimeValue(absoluteValue);

    // Validação final (agora sem verificar se hours < 0)
    if (isNaN(hours) || isNaN(minutes) || minutes < 0 || minutes > 59) {
        return '-';
    }

    // Formata a saída
    const formatted = formatTimeOutput(hours, minutes);
...
const parseTimeValue = (value: string | number): { hours: number; minutes: number } => {
    ...
    if (typeof value === 'number') {
        return parseDecimalTime(Math.abs(value)); // Use Math.abs aqui
    }
...
const formatTimeOutput = (hours: number, minutes: number): string => {
    ...
    return hours === 1 ? `1h:${minutesStr}min` : `${hoursFormatted}hs:${minutesStr}min`;
};
// ====================================================================================================
